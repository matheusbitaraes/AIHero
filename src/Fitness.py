...
class Fitness:
    def __init__(self, w1=50, w2=25, w3=100, w4=-25, w5=12, w6=50, w7=25, w8=0, scale='minor_blues_scale'):
        self.w1 = w1
        self.w2 = w2
        self.w3 = w3
        self.w4 = w4
    ...
    def eval(self, note_sequence, chord_notes):
        ...
        f2 = self.notesOnTime(note_sequence)  # porcentagem de notas nos tempos ou contra tempos (0-1)
        ...
        fitness = self.w1 * f1 + self.w2 * f2 + self.w3 * f3 + self.w4 * f4 + \
                  f5 + self.w6 * f6 + self.w7 * f7  # + self.w8 * f8 todo: implement for future enhancements
    ...
    #
    #     # iterate through known licks
    #     global_similarity = 0
    #     for lick in known_licks_of_scale:
    #         lick_similarity = self.getSequenceSimilarity(note_sequence, lick)
    #         if lick_similarity > global_similarity:
    #             global_similarity = lick_similarity
    #
    #         # # iterate into melody array matching lick size
    #         # for i in range(0, math.floor(len(note_sequence)/len(lick))):
    #         #     a = i*len(lick)
    #         #     b = (i+1)*len(lick)
    #         #     note_fraction = note_sequence[a:b]
    #         #     lick_similarity = self.getSequenceSimilarity(note_fraction, lick)
    #         #     if lick_similarity > global_similarity:
    #         #         global_similarity = lick_similarity
    #
